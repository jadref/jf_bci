  ...
  elseif( numel(spType)==1 ) %single input=set of classes to make a 1vR decomp for
	 ...
	 if(ischar(tmp) && any(strncmp(tmp,{'%','#'},1)) && any(strncmp(tmp(end),{'%','#'},1))) 
		...
		for ci=1:numel(classNms);
		  si=regexp(classNms{ci},tmp(2:end-1),'once'); if(iscell(si))si=si{1};end;
		  if( ~isempty(si) ) t(ci)=true; end;
	   end
...
for i=1:numel(spNms);
  ...
  if ( any(strncmp(spNms{i},{'%','#'},1)) && any(strncmp(spNms{i}(end),{'%','#'})) ) % use regexp match
    ...
    t=strcmp(tgt,classNms);
    if ( ~any(t) ) % try just matching the start of the class name
       fprintf('Warning: couldnt match full class name, trying just the prefix');
