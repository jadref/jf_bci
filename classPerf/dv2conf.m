...
%
% Example:
%  reshape(dv2conf(Y,dv),size(Y,2),size(Y,2),[]) % confusion matrix for each entry in dv
%
% TODO: [] fix bug dependent on order of inputs, i.e dv2conf(Y,dv)~=dv2conf(Y',dv',[2 1])
...
if ( size(Y,2)==size(dv,dims(1)) ) Y=Y'; end; % ensure Y has sub-probs in 2nd dim
if ( size(Y,1)~=size(dv,dims(1)) || ...
     (dims(2)>0 && size(Y,ndims(Y))~=size(dv,dims(2))) ) 
   error('decision values and targets must be the same number of trials');
end
Y(isnan(Y(:)))=0; % ensure NaN are ignored
...
if ( ~islogical(Y) ) % convert to logical predictions
  if ( all(Y(:)==0 | Y(:)==-1 | Y(:)==1 | isnan(Y(:))) && size(spMx,2)==size(spMx,1) )
	 Y=(Y>0);
  else
	 exInd= all(Y==0 | isnan(Y),ndims(Y)); % indices to ignore
	 Y    = dv2pred(Y,ndims(Y),spMx,varargin{:});
	 % ensure ignored have 0 value
	 Y(repmat(exInd,[ones(1,ndims(Y)-1) size(Y,ndims(Y))]))=0;
  end
end
if ( ~islogical(pred) ) % convert to logical predictions
  pred = dv2pred(dv,dims(end),spMx,varargin{:});
end
