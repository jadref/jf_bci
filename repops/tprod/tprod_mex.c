...
void mexFunction(const int nlhs, mxArray *plhs[], 
  ...
  if( nrhs >= 5 && mxIsChar(prhs[4]) ) {
	 ...
	 for (i=0 ; opnmStr[i] != 0; i++ ) {
		switch ( opnmStr[i] ) {
		case 'm': case 'M':   useMATLAB=0;  break;
  ...
  #ifdef LOGGING  
  fprintf(fd," = [");printMxInfoSummary(fd,zinfo);fprintf(fd,"]");  
  #endif
...
mxArray* MATLAB_mm(MxInfo zinfo, const MxInfo xinfo, const MxInfo yinfo,
  ...
  /* BODGE: I suspect this is the cause of some of the memory issues
	  we have when using tprod, better to use the mxArray structures
	  and matlabs copyMatrix function. */
  ...
  mxSetDimensions(Zmx,zinfo.sz,zinfo.nd); /* re-shape z to the output size */
  /* Set trap so errors return here so we can clean up correctly */
  mexSetTrapFlag(0);
