...
if ( nargin < 1 ) X=[]; end;
if( nargin<2 || isempty(Y) ) if(~ischar(X)) Y=randn(size(X)); else Y=[]; end;end
if ( nargin < 3 || isempty(sz) ) sz=[11 29 51]; end;
if ( isempty(X)||ischar(X) || isempty(Y)||ischar(Y) ) 
   if (ischar(X)) X=feval(X,randn(sz)); else X=randn(sz); end;
   if (ischar(Y)) Y=feval(Y,randn(sz)); else Y=randn(sz); end;
end
