...
if(isa(X,'single')) eps=1e-7; minp=1e-34; maxf=single(-log(minp)); else eps=1e-16; minp=1e-120; maxf=-log(minp); end;
...
if ( isempty(wb) )    
  ...
  alpha= Y; alpha(Y<=0)=0; if ( size(Y,1)==1 ) alpha=[Y;Y]; alpha(Y<=0,1)=0; alpha(Y>=0,2)=0; end
  alpha= repop(alpha,'./',max(1,sum(alpha,2))); % normalize to unit mag with guard divide by 0
  Xmu  = X*alpha'; % centroid of each class
  % direction is center from center of other classes to centroid of the tgt-class 
  w=zeros(size(Xmu));
  for ci=1:size(Xmu,2);
	 w(:,ci) = Xmu(:,ci)-mean(Xmu(:,[1:ci-1 ci+1:end]),2);
  end
...
%    = -sum_i log(p(p_true));  % when have hot-one (true/false) encoded input
   ...
   for j=1:opts.maxLineSrch;
      ...
      if ( 0 ) 
		...
		if ( ~opts.rescaledv && isnan(dtdJ) ) % numerical issues detected, restart
		  if (opts.verb>1) fprintf('%d) Numerical issues falling back on re-scaled dv\n',iter);end;
...
% post-process to remove any systematic offset in the predictions..
offset=mean(f(:)); f=f-offset; b=b-offset;
