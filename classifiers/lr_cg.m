...
if ( ~isempty(opts.dim) && any(opts.dim<ndims(X)-numel(opts.dim)+1) ) % permute X to make dim the last dimension
   persistent warned
   if (isempty(warned) ) 
...
szX=size(X); nd=numel(szX); if( isempty(opts.dim) ) opts.dim=nd; end; N=prod(szX(opts.dim)); nf=prod(szX(1:end-numel(opts.dim)));
szY=size(Y); szY(end+1:numel(opts.dim)+1)=1;
if(size(Y,1)==1) Y=Y'; end; % ensure Y is col vector
Y(isnan(Y))=0; % convert NaN's to 0 so are ignored
L=size(Y,numel(opts.dim)+1);
...
tic,ls_cg(X,Y,0,'verb',1,'objTol0',1e-10,'wb',wb0);toc


% now with indicator for Y
Yi=single(cat(2,Y(:)>0,Y(:)<0));
tic,ls_cg(X,Yi,0,'verb',1,'objTol0',1e-10,'wb',wb0);toc
% now with example weighting
wght  = .5+rand(size(Yi,1),1)*.5;
tic,[wb]=ls_cg(X,repop(Yi,'*',wght),0,'verb',1,'objTol0',1e-10,'wb',wb0);toc
% now heavily weighted universum examples
Ywght=Yi; Ywght(1:10,:)=6;
tic,[wb]=ls_cg(X,Ywght,0,'verb',1,'objTol0',1e-10,'wb',wb0);toc

clf;plotLinDecisFn(X,Y,wb(1:end-1),wb(end));


K=X'*X; tic, [alphab,f,J]=kls_cg(K,Y,0,'verb',1,'objTol0',1e-10);toc
wb=[X*alphab(1:end-1);alphab(end)];
dvk=wb(1:end-1)'*X+wb(end);

tic,[wb,f,Jlr]=ls_cg(X,Y,0,'verb',1,'objTol0',1e-10);toc
...
ls_cg(X,Y,0,'wPC',1,'bPC',1); % no PC
ls_cg(X,Y,0,'PCmethod','wb0')% zeros PC
ls_cg(X,Y,0,'PCmethod','wb') % seed solution
