function [wb,f,J,obj,tstep]=mlr_cg2(X,Y,R,varargin);
...
if ( isempty(wb) )    
  ...
  alpha= single(Y>0); alpha=repop(alpha,'./',sum(alpha,2));
...
Yerr = Y-repop(p,'*',sY);  % [LxN] = dln(p_y), i.e. gradient of the log probability of the true class
...
for iter=1:min(opts.maxIter,2e6);  % stop some matlab versions complaining about index too big
   ...
   for j=1:opts.maxLineSrch;
      ...
      wX   = wX0+tstep*dX;%w'*X;
		...
		p    = repop(p,'/',sum(p,1)); % [L x N] =Pr(x|y_i) = exp(w_ix+b)./sum_y(exp(w_yx+b));
		% g    = p(Yidx); % [1xN] = Pr(y_true)
		%Yerr = Yind-p;  % [LxN] = dp_y, i.e. gradient of the log probability of the true class
		Yerr = Y-repop(p,'*',sY);  % [LxN] = dln(p_y), i.e. gradient of the log prob true class
		Yerr(:,exInd)=0;% ensure excluded are ignored
      dtdJ  = -(2*(dRw+tstep*dRd) + dmu - dX(:)'*Yerr(:) - db(:)'*sum(Yerr,2));
...
nd=100; nClass=800;
[X,Y]=mkMultiClassTst([zeros(1,nd) -1 0 zeros(1,nd); zeros(1,nd) 1 0 zeros(1,nd); zeros(1,nd) 0 1 zeros(1,nd); zeros(1,nd) 0 -1 zeros(1,nd)],[nClass nClass nClass nClass],[.4 .4],[]);[dim,N]=size(X);

clf;labScatPlot(X,Y,'linewidth',1)

wb0=rand(size(X,1)+1,4);
tic,[wb,f,Jlr]=mlr_cg(X,Y,0,'verb',1,'wb',wb0);toc
% with label indicator
Yi=lab2ind(Y);
[wb,f,Jlr]=mlr_cg(X,Yi,0,'verb',1,'wb',wb0);
% with repeated labels
[wb,f,Jlr]=mlr_cg(X,cat(2,Yi,Yi),0,'verb',1,'wb',wb0);
