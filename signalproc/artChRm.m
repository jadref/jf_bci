...
opts=struct('detrend',0,'center',0,'bands',[],'fs',[],'maxIter',0,'tol',.02,'verb',0);
...
for iter=0:opts.maxIter;
  ...
  if ( numel(artCov)>1 ) % whiten artifact signal
     ...
     si = S>=max(abs(S))*(opts.tol).^2 & abs(S)>eps; S=S(si); U=U(:,si); % remove degenerate parts
...
%  N.B.  X' = X(I-artSig*artSig') = X - X*artSig*artSig' = X - sf*artSig  iff artSig*artSig=I
%  now, if artSig = sf'*X then we also have: X' = X-sf*sf'*X = (I-sf*sf')*X
%  thus we can remove with just a simple spatial filter
