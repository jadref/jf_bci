function [x,s]=biasFilt(x,s,alpha,accdim,verb)
% bias removing filter (high-pass) removes slow drifts from inputs
%
%   [x,s,mu,std]=biasFilt(x,s,alpha,accdim)
%
% Inputs:
...
% bias adapt
if( isempty(accdim) ) x=      x  -  s.sx./s.N(:,1); 
else                  x=repop(x,'-',s.sx./s.N(:,1)); 
end
...
% simple test
s=[];fs=[];fx=[];for i=1:size(x,2); [fx(:,i),si]=biasFilt(x(:,i),s,100); fs(:,i)=si.sx/si.N; s=si; end;
% feature specific smoothers
s=[];fs=[];fx=[];for i=1:size(x,2); [fx(:,i),si]=biasFilt(x(:,i),s,[exp(log(.5)/100);exp(log(.5)/1000)]); fs(:,i)=si.sx/si.N; s=si; end;
...
s=[];fs=[];ss=[];fx=[];for i=1:size(x,2); [fx(:,i),si]=biasFilt(x(:,i),s,[100 20]); fs(:,i)=si.sx/si.N(:,1); ss(:,i)=si.x./si.N(:,2); s=si; end;
...
s=[];fs=[];fx=[];for i=1:size(x2d,3); [fx(:,:,i),si]=biasFilt(x2d(:,:,i),s,100,2); fs(:,i)=si.sx/si.N; s=si; end;
