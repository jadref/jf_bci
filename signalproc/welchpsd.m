...
%                'amp'/'abs' - ave amplitude, 'power' - ave power, 'db' - ave db
%  outType    -- 'str' type of output to produce.  one of:
%                'amp'/'abs' - ave amp, 'power' - ave power, 'db' - ave db
...
if(~ischar(opts.windowType) && ~isequal(opts.windowType,1) && isempty(opts.width_samp) ) opts.width_samp=numel(opts.windowType); end; 
...
[start width]=compWinLoc(size(X,dim),opts.nwindows,opts.overlap,opts.start_samp,opts.width_samp,opts.step_samp);
...
for wi=1:numel(start);
   ...
   if(~isempty(taper)) wX = repop(wX,'.*',taper);       end; % window
   ...
   switch(lower(aveType));
    ...
    case {'pow','power'}; W = W + wX;                          % map to power
    case {'abs','amp'};   W = W + sqrt(wX);                    % map to amplitudes
...
st=1; if ( ~isempty(taper) ) st=sum(taper); end;
W=W./(numel(start)*st);
return;
