...
if ( nargin < 4 ) wght=1; end; wght=wght(:);
...
xbw = xb; if ( ~isempty(wght) ) xbw = repop(xb,'.*',wght); end % include weighting effect
...
npre=100; npost=100;  wght=[ones(1,npre) zeros(1,size(f,1)-npre-npost) ones(1,npost)];
ff=detrend(f,1,[],wght); % weighted
...
clf; plot([f(:,1) ff(:,1)],'linewidth',2); hold on;
