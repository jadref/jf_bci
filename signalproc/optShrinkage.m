...
%  X  - [n-d] inputs 
%     OR
%       [int] N number of samples used to compute summary statistics. 
%         N.B. in this case you *must* supply X2 (and muX XmuX if centering wanted)
%  dim - dimension(s) of X which contain the channels
%  Sigma - pre-computed channel covariance matrix
%  X2    - [float] pre-computed sum squared sample length: sum(vec(sum(X.^2,dim).^2)) ([])
%       OR 
%          [n-d] pre-computed squared sample length:              sum(X.^2,dim).^2)
%  centerp - [bool] center the data (1)
%  muX   - pre-computed channel mean
%  XmuX   - pre-computed mean channel times channel mean
...
if ( nargin<6 ) muX=[]; end;
if ( nargin<7 ) XmuX=[]; end;
if ( numel(X)==1 ) 
   N=X;
else % X is actual data, compute useful stuff
   szX=size(X); 
   if ( ndims(X)>2 )
   ...
   if ( centerp && isempty(XmuX) )
      idx=1:ndims(X); idx(dim)=-dim; XmuX=tprod(X,idx,muX(:),-dim,'n');
   end
end

[lambda,beta2,alpha2]=innerOptShrinkage(Sigma,X2,N,muX,XmuX);
return

function [lambda,beta2,alpha2]=innerOptShrinkage(Sigma,X2,N,muX,XmuX)
% [lambda,beta2,alpha2]=inneroptShrinkage(Sigma,X2,N,muX,XmuX)
%
% Inputs:
%  Sigma - pre-computed channel covariance matrix
...
%       OR 
%          [n-d] pre-computed squared sample length:              sum(X.^2,dim).^2)
%  N     - [int] number of examples used to compute Sigma and X2
%  muX   - pre-computed channel mean
%  XmuX   - pre-computed mean channel times channel mean
if( nargin<4 ) muX=[]; end;
if( nargin<5 ) XmuX=[]; end;
...
if (~isempty(muX)) %beta^2=E(|S-\Sigma|_F^2)~=1/N^2 \sum_i(|x_ix_i' - Sigma|_F^2), expected error to sample estimate
