...
if ( numel(xscale)==1 && isnumeric(xscale) )  xscale=(1:len)*xscale; end; 
   ...
   for i=1:numel(bands)
      ...
      if ( numel(bands{i})==4 ) % use xscale to convert to indicies in the fft'd data array
        ...
        for j=1:numel(bands{i}); 
          if ( isnumeric(xscale) )
				band(j)      = max(min(bands{i}(j),xscale(end)),xscale(1)); % deal with stuff outside the limits
            [ans,band(j)]= min(abs(band(j)-xscale)); % and inside the limits
