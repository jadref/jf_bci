...
if ( nargin < 2 || isempty(key) ) % default key
  if ( iscell(Y) && ~ischar(Y{1}) ) key=unique([Y{:}]); else key=unique(Y(:)); end;
  key=key(:)'; 
...
if ( isnumeric(Y) )
  ...
  for i=1:numel(Y);
    for ikey=1:numel(key);
		if ( iscell(key) ) keyikey=key{ikey}; else keyikey=key(ikey); end;
		if ( isequal(Y{i},keyikey) ) ind(i,:)=spMx(:,ikey); end;
    end;
  end
end
