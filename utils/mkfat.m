function [x,fat]=mkfat(varargin)
% [x,fat]=mkfat(varargin)
...
% e.g.  [x,fat]=fatvec('hello',eye(3,3),'there',randn(10,1));
%       hello=x(fat.hello); there=x(fat.there);
...
for i=strt:2:numel(varargin);   
   ...
   % save ram by using short ints for the index when we can
   if ( lst+n < intmax('int16') ) % N.B. int is 2x faster idx
      fat.(varargin{i})=int16(reshape([1:n]+lst,sz));
   else
      fat.(varargin{i})=int32(reshape([1:n]+lst,sz)); 
   end
   lst=lst+n;
end
