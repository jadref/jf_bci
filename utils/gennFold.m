...
elseif ( isequal(nFold,'loo') || isequal(nFold,'lno') || isequal(opts.foldSize,1) ) % Leave-one-out / leave-n-out
  ...
  for fi=1:numel(fIdxs)-1; foldIdxs(fIdxs(fi)+1:fIdxs(fi+1),fi)=1; end;
  if ( numel(dim)>1) foldIdxs=reshape(foldIdxs,szY(setdiff(1:end,dim)),[]); end;
  if ( foldSize<0 ) foldIdxs=-foldIdxs; end;
  return;
elseif ( isequal(nFold,'llo') ) % leave label out
  key=unique(Y(:));
  if(~opts.zeroLab && isnumeric(Y)) key(key==0)=[]; end; % remove zero labels from consideration
...
if( dim<ndims(Y) ) % transpose if sub-probs in the 1st dimension
...
if ( ~opts.zeroLab ) Yu(all(Yu==0,2),:)=[]; end; % remove zero labelled points
...
% #3: normal folding
  ...
  if ( abs(nFold) > size(Y,1) ) warning('More folds than datapoints!'); end;
