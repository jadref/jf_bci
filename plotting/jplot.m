...
opts=struct('xs',[],'ys',[],'layout',[],'clim','minmax','electrodes',4,'colorbar',1,...
				'interpMethod','invdist','xlabel',[],'ylabel',[],'labels',[],'alphaMask',0,...
				'maskVal',0,'padfactor',1.1,'head',2,'contour',2,'rmax',[]);
[opts,varargin]=parseOpts(opts,varargin);
if(~isempty(opts.labels) && ~iscell(opts.labels) && ischar(opts.labels)) opts.labels={opts.labels}; end;
if(~isempty(opts.xlabel) && ~iscell(opts.xlabel) && ischar(opts.labels)) opts.xlabel={opts.xlabel}; end
if(~isempty(opts.ylabel) && ~iscell(opts.ylabel) && ischar(opts.labels)) opts.ylabel={opts.ylabel}; end
...
for i=1:nfilt;
   ...
   if ( ~isempty(opts.electrodes) && ~isequal(opts.electrodes,0) ) 
      ...
      if( ischar(opts.electrodes) || (iscell(opts.electrodes) && ischar(opts.electrodes{1})) ) % electrode names
