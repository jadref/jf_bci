...
format = { % format string for u8read/u8write
  'b'
  'B'
  'S'
  'l'
...
while ~feof(F)
  ...
  if value_type==0
    ...
    if ( exist('typecast','builtin') )
      E(num).value = typecast(buf((type_size+1):(type_size+value_size)), type{value_type+1});
    elseif ( exist('u8read','file') )
      
      E(num).value = u8read(format{value_type+1},buf((type_size+1):(type_size+value_size)));
    end
  end
end
