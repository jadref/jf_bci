      ...
      if ( isempty(sessdirs) ) 
        if( numel(sessions)==1 && isempty(sessions{1}) )
          ...
          sessdirs={'.'}; sessmatch=0;
				 ...
				 % try just matching the path directly
			 for si=1:numel(sessions);
				if ( exist(fullfile(subjdir,sessions{si}),'file') )
				  sessdirs={sessdirs{:} sessions{si}}; sessmatch(numel(sessdirs))=si;
				end
			 end
        end;
		  if ( opts.verb > 0 ) fprintf('\t\tmatched %d sessdirs: %s\n',numel(sessdirs),sprintf('%s, ',sessdirs{:})); end;
		  if ( isempty(sessdirs) ) continue; end;
      end
   else
      sessdirs={'.'}; sessmatch=0;
   end
         ...
         if ( isempty(blkdirs) ) 
           if( isempty(blocks) || (iscell(blocks) && numel(blocks)==1 && isempty(blocks{1})) )
             ...
             blkdirs={'.'}; blkmatch=0;
				 ...
				 for si=1:numel(blocks);
					if ( exist(fullfile(subjdir,sessdir,blocks{si}),'file') )
					  blkdirs={blkdirs{:} blocks{si}}; blkmatch(numel(blkdirs))=si;
					end
				 end
           end;
         end
         if( opts.verb>0 ) fprintf('\t\tmatched %d blkdirs: %s\n',numel(blkdirs),sprintf('%s, ',blkdirs{:})); end;
			if ( isempty(blkdirs) ) continue; end;
         % Record which block are in which session directories
         if ( isempty(sessmatch) || sessmatch(sessi)==0 ) sess=''; 
         else 
      ...
      for fi=1:numel(fnames); % loop over each file testing its validaty
         ...
         if ( ~isempty(opts.blkfileregexp) )
           if ( isempty(blocks) ) 
             ...
             if (~isempty(matchstrs)&&~isempty(matchstrs{1})) fnameinfo.block=matchstrs{1}{1}{1};end
