...
% identify outlying elements in a matrix using robust variance/mean computation 
%
...
for iter=1:maxIter;
   ...
   mufeat(iter)  = median(feat(~badInd)); % robust center
   stdfeat(iter) = sqrt(mean((feat(~badInd)-mufeat(iter)).^2)); % robust length scale = root median squared distance
...
function testCase()
X=randn(1000,1);
oI=randn(size(X))>1; X(oI)=X(oI)*5; % insert outliers
bad1=idOutliers(X); % one-sided
bad2=idOutliers(X,[],[3.5 3.5]); % two-sided
clf;plot([X bad1*5 -bad2*5],'linewidth',2);
(oI'*[bad1 bad2])./(oI'*oI) % correlation between detected and true bad
