function v = logdet(A, use_chol, tol)
...
if nargin < 2
    ...
    assert(any(strcmpi(use_chol, {'chol','cholp'})), ...
        'logdet:invalidarg', ...
...
if strcmpi(use_chol,'chol')
   dC= diag(chol(A));
   if ( tol==0 )
      v = 2 * sum(log(dC));
   else % truncated chol computation
      if ( tol<0 ) tol=max(dC)*abs(tol); end;
      v = 2 * sum(log(max(dC,tol)));
   end
else
   [L, U, P] = lu(A);
   du = diag(U);
